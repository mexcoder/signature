...
class product(baseSignature):

    # properties

    @property
    def modulo(self):
        """this modulo, used on all cryptographic operations"""
        return self._modulo
    ...
    def makeKeys(self, private=None, modulo=None):
        ...
        # by the formula ´a * b mod c = 1´ where a is the private key, b is the 
        # public key and c is the field or modulo of the operation so the 
        # identity would be 1.
        # the public key must be computed using the Extended Euclidean algorithm
        #
        # this is not a secure formula and is only intended for academic use
        # as the modulo is public one could derive the private key, ideally it
        # should be way harder to do this, RSA for an instance is based on the
        # product of to large prime numbers and the exponentiation opetarion,
        # this makes deriving the private key from the public key almost
        # impossible
        self.public_key = mod_inverse(self.private_key, self.modulo)
...
def main():
    ...
    big_prime = 10142652451601569649 # this is a 64 bit prime number
    instance = product(modulo=big_prime)
